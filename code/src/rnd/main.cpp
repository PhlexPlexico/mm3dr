...
void Init(Context& context) {
  msys::Watches_Init();
  util::Print("MM3D Rando initialised (" __DATE__ " " __TIME__ ")");
...
RST_HOOK void Calc(game::State* state) {
  ...
  if (false) {
    ...
    // if (link) {
    //   rnd::util::Print("Common data picto ptr is %p, action for blast mask is %u", &(link->active_mask_id), (u8)tmp);
    // }
...
RST_HOOK void DrawMenu() {
  ...
  if (showTitle) {
    Draw_DrawFormattedStringTop(150, 20, COLOR_WHITE, "MM3D Randomizer");
