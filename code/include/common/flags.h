...
namespace rnd {
  ...
  template <typename FlagType, typename = typename std::enable_if_t<std::is_enum_v<FlagType>, void>> class Flags {
  public:
    constexpr auto& operator=(FlagType v) {
      flags = std::underlying_type_t<FlagType>(v);
      return *this;
    }
    constexpr void Set(FlagType v) {
      flags |= std::underlying_type_t<FlagType>(v);
    }
    constexpr void Clear(FlagType v) {
      flags &= ~std::underlying_type_t<FlagType>(v);
    }
    ...
    template <typename... Flags> constexpr bool AreAllSet(Flags... v) const {
      return (... && IsSet(v));
    }

    template <typename... Flags> constexpr bool IsOneSet(Flags... v) const {
      return (... || IsSet(v));
    }
  ...
  template <size_t N, typename WordType = u32, typename IndexType = WordType> class BitSet {
  public:
    constexpr void Set(IndexType idx) {
      GetWord(idx) |= 1 << (size_t(idx) % NumBitsPerWord);
    }
    constexpr void Clear(IndexType idx) {
      GetWord(idx) &= ~(1 << (size_t(idx) % NumBitsPerWord));
    }
    ...
    constexpr WordType GetStorage(size_t idx) const {
  ...
  private:
    ...
    constexpr WordType& GetWord(IndexType idx) {
