...
static const StateInfo* FindStateInfo(Predicate predicate) {
  const auto* table = rnd::util::GetPointer<StateInfo>(0x6883FC);
  ...
  rnd::util::Print("%s: ERROR - unknown game state. The game will break now.", __func__);
  return nullptr;
}
...
extern "C" const StateInfo* rst_GameStateGetNextStateInfo(State* state) {
  ...
  rnd::util::Print("%s: looking up new game state for type %u", __func__, u8(state->type));
...
void SaveFile::Open() {
  rnd::util::GetPointer<void(SaveFile*)>(0x18DA50)(this);
}

bool SaveFile::WriteAndFree() {
  return rnd::util::GetPointer<bool(SaveFile*)>(0x18AD44)(this);
}
