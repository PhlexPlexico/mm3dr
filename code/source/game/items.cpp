...
namespace game {
  ...
  Action GetActionForItem(ItemId item) {
    ...
    if (u8(item) >= std::size(table)) return Action(0xffffffff);
  ...
  bool CanUseItemUnsafe(ItemId item_id) {
    ...
    if (player->action_type == act::Player::ActionType::Type2) return false;
  ...
  bool CanUseItem(ItemId item) {
    ...
    if (item == ItemId::Ocarina) return cdata.usable_btns[u8(UsableButton::Ocarina)] != ButtonIsUsable::No;

    if (item == ItemId::PictographBox) return cdata.usable_btns[u8(UsableButton::PictographBox)] != ButtonIsUsable::No;
    ...
    if (cdata.save.anonymous_13 & 2) return false;

    if (cdata.save.anonymous_138 & 0x20) return false;

    if (cdata.save.anonymous_130 & 8) return false;

    // Cutscene map
    if (gctx->scene == SceneId::Cutscenes && cdata.setup2 == 6) return false;

    if ((cdata.save.anonymous_12 & 0x10) && !player->flags3.IsSet(act::Player::Flag3::DekuNutsOnB)) return false;

    if (player->flags3.IsSet(act::Player::Flag3::DekuNutsOnB)) return false;
    ...
    if (player->flags1.IsSet(act::Player::Flag1::Unk200000)) return item == ItemId::LensOfTruth;
    ...
    if (player->id != act::Id::Player) return false;

    if (player->active_mask_id == MaskId::GiantMask) return item == ItemId::GiantMask;

    if (item == ItemId::GiantMask) return gctx->scene == SceneId::TwinmoldLair;

    if (item == ItemId::FierceDeityMask) return IsBossRoomScene(gctx);

    if (gctx->actors.field_4) {
      if (rnd::util::IsAnyOf(item, ItemId::FireArrow, ItemId::IceArrow, ItemId::LightArrow)) return false;
    }

    if (ItemIsGear(item) || ItemIsBottled(item)) return !gctx->hud_state.prohibit_gear_or_bottles;

    if (ItemIsMask(item)) return !gctx->hud_state.prohibit_masks;
