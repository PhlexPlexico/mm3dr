...
namespace rnd {
  ...
  void calc(game::State* state) {
    ...
    if (!context.has_initialised && state->type == game::StateType::FirstGame) Init(context);
    if (state->type != game::StateType::Play) return;
...
// Before calling let's be absolutely sure we have the player available.
    ...
    if (context.gctx->pad_state.input.buttons.IsSet(game::pad::Button::ZL)) {
      ...
      if (link) {
        ...
        // rnd::util::GetPointer<void(game::act::Actor*, game::GlobalContext*)>(0x3b9c2c)((game::act::Actor*)actor,
        // context.gctx); rnd::util::GetPointer<void(game::act::Actor*,
        // game::GlobalContext*)>(0x4bf7b8)((game::act::Actor*)actor, context.gctx);
        // rnd::util::GetPointer<void(game::act::Actor*, game::GlobalContext*)>(0x4bfab4)((game::act::Actor*)actor,
        // context.gctx); rnd::util::GetPointer<void(game::act::Actor*, game::GlobalContext*, int,
...
#endif
    if (context.gctx->GetPlayerActor()) ItemOverride_Update();
    return;
  }
  void readPadInput() {
    auto* gctx = GetContext().gctx;
    if (!gctx || gctx->type != game::StateType::Play) return;
